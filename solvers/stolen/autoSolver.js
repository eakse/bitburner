import { getAllServerNames } from "/util.js";


function manualTry(ns) {
    return
    let inputstr = "nwo contract-296884-FulcrumTechnologies.cct";
    let server = inputstr.split(" ")[0];
    let contract = inputstr.split(" ")[1];

    let solution = 1;
    let res = ns.codingcontract.attempt(solution, contract, server, { "returnReward": true });
    ns.tprint(res);
}
...
export function main(ns) {
    ...
    const contracts = getAllServerNames(ns).flatMap((server) => {
...
function solve(type, data, server, contract, ns) {
    ...
    switch (type) {
        ...
        case "Subarray with Maximum Sum":
            solution = subarrayWithMaximumSum(data);
            break;
    }
